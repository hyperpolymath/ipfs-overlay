...
All notable changes to `ipfs-overlay` will be documented in this file.
...
- fix(ci): hypatia-scan workdir (${{ env.HOME }} resolves empty) (#51)
- fix(ci): hypatia-scan.yml -- --exit-zero + GITHUB_TOKEN (hyperpolymath/hypatia#213) (#46)
- fix(ci): bump erlef/setup-beam SHA for ubuntu24 runner support (#47)
- fix(ci): repair YAML block-scalar in workflow-linter Check Permissions step (#48)
- fix(ci): move secret-scanner Cargo.toml gate from job-level if: to step-level (#49)
...
- ci: redistribute concurrency-cancel guard to read-only check workflows (#57)
- ci(secret-scanner): drop duplicate --fail from trufflehog extra_args (#45)
